...
class CrearCuentaVC: UIViewController {
        ...
        manager.logIn(readPermissions: [.publicProfile], viewController: self) { (result) in
            ...
            case .success:
                ...
                Auth.auth().signIn(with: credential) { (user, error) in
                    if let error = error {
                        print("CTM ERROR AUTH")
                        print(error)
                        print(error._code)
                        //self.handleError(error)      // use the handleError method
                        if error._code == 17012 {
                            print("SI ENTRA ERROR 17012")
                            let alert = UIAlertController(title: "Correo ya utilizado", message: "El correo asociado a esta cuenta de Facebook ya ha sido registrado mediante el metodo de inicio de Sesión de Usuario y Contraseña.", preferredStyle: UIAlertController.Style.alert)
                    ...
                    DispatchQueue.main.asyncAfter(deadline: deadlineTime, execute: {
                        print("C")
                        self.performSegue(withIdentifier: "empezarFBSegue", sender: self)
                    })
    ...
    func crearUsuario(){
                ...
                if let picture = result["picture"] as? Dictionary<String, Any> {
                    if let pictureData = picture["data"] as? Dictionary<String, Any> {
                        if let pictureURL = NSURL(string: (pictureData["url"] as? String)! ) {
                            let urlFotoFB = pictureURL
                            if let data = try? Data(contentsOf: urlFotoFB as URL)
                            {
                               // fotoFB = UIImage(data: data)!
                                dataFotoFB = data as NSData
                            }
        ...
        let pes = Peso()
        pes.pesoKg = self.peso
        pes.fechaRegistro = self.fechaStringADate(fechaString: Date().string(format: "dd/MM/yyyy"))
        ...
        persona.nombre = nombreFB
        persona.correo = Auth.auth().currentUser!.email!
        persona.foto = dataFotoFB
        persona.altura = self.altura
        persona.sexo = self.sexo
            
        persona.pesos.append(pes)
        persona.dieta.append(die)
            ...
            let item51 = AlimentosBase ();item51.id = 51;item51.nombre = "Avena";item51.tipoMacro = "Carbohidrato";item51.subMacro = "Avena";item51.cal = 62.2;item51.prot = 1.9;item51.carb = 11.0;item51.fat = 1.2;item51.unidadPorcion = "1 cuchara";item51.porcionMin = 2;item51.porcionMax = 4;item51.desayuno = true;item51.almuerzo = false;item51.cena = false;item51.snack = false
    ...
    func fechaStringADate(fechaString: String) -> Date? { //recibe fecha en formato dd/MM/yyyy
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "emailSegue") {
            ...
            var vc = segue.destination as! CuentaEmailVC
        ...
        } else if (segue.identifier == "empezarFBSegue") {
